...
function ForgotPassword() {
  ...
  async function submit(e: React.FormEvent) {
    ...
    try {
      ...
      if (err) throw new Error(err.message || "Could not send a reset link.");
      setDone(true);
    } catch (err) {
    ...
    <main className="container-pg grid min-h-0 flex-1 place-items-center py-10 md:py-14">
      <div className="w-full max-w-md rounded-2xl bg-cream p-6 shadow-[var(--shadow-card)] sm:p-8">
        ...
        <p className="mt-2 text-sm text-muted">
          If an account exists for this email, Darmelk will send a reset link.
        </p>
        {done ? (
          <div className="mt-6 space-y-4">
            <SuccessBanner
              title="Request sent"
              description="Check that inbox. If no email arrives, try again or contact Darmelk."
            />
          ...
          <form onSubmit={submit} className="mt-6 space-y-4">
            <Field label="Email" error={error ?? undefined} htmlFor="forgot-email">
              <Input
                id="forgot-email"
                type="email"
                required
