...
export type FoundationScope = {
  ...
  flagshipPresent: boolean;
  catalogOfferCount: number;
  migrationCount: number;
  dependentRecords: Record<string, number>;
};

export function assessCleanFoundationPreconditions(scope: FoundationScope): { ok: boolean; errors: string[] } {
  const errors: string[] = [];
  if (scope.admins.length === 0) errors.push("no admin members preserved");
  if (scope.nonAdminCount !== 0) errors.push(`non-admin members present (${scope.nonAdminCount})`);
  if (scope.foundationExisting !== 0) errors.push(`foundation members already exist (${scope.foundationExisting})`);
  if (scope.orphanAuthUsers !== 0) errors.push(`orphan auth users present (${scope.orphanAuthUsers})`);
  if (!scope.flagshipPresent) errors.push("flagship offer missing");
  if (scope.catalogOfferCount < 1) errors.push("catalog offers missing");
  if (scope.migrationCount < 1) errors.push("migrations table empty");
  return { ok: errors.length === 0, errors };
}

export async function inspectResetScope(client: PoolClient): Promise<FoundationScope> {
  ...
  const flagshipPresent = (await countSql(
    client,
    `select count(*)::text as count from offers where slug = $1`,
    [FLAGSHIP_SLUG],
  )) > 0;
  const catalogOfferCount = await countSql(client, `select count(*)::text as count from offers`);
  const migrationCount = await countSql(client, `select count(*)::text as count from _migrations`);
